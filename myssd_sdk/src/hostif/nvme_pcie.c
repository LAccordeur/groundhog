                            ...
                            int rx_intr, int tx_intr)
{
    ...
    if (status != XST_SUCCESS) {
        xil_printf("Initialization dfadfa failed %d\r\n", status);
        return XST_FAILURE;
    }
    ...
    status = intr_setup_irq(ctlr->tx_intr, 0x3,
                            (irq_handler_t)tx_dma_intr_handler, ctlr);
    if (status != XST_SUCCESS) {
...
static inline void __nvme_pcie_post_cqe(struct nvme_pcie_ctlr* ctlr, u16 sqid,
                                        u16 status, u16 cmdid, u32 result)
{
    nvme_pcie_writel(ctlr, NVME_PCIE_CQ_FIFO_STATUS_REG,
                     (status & 0xffff) | ((cmdid & 0xffff) << 16));
    nvme_pcie_writel(ctlr, NVME_PCIE_CQ_FIFO_SQID_REG, sqid & 0xffff);
    nvme_pcie_writel_release(ctlr, NVME_PCIE_CQ_FIFO_RESULT_REG, result);
}
...
void nvme_pcie_post_cqe(u16 sqid, u16 status, u16 cmdid, u32 result)
{
