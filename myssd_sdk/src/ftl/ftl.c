...
static int segment_user_request(struct user_request* req)
{
    ...
    while (count < req->sector_count) {
        ...
        txn->req = req;
        txn->type = (req->req_type == IOREQ_READ || req->req_type == IOREQ_EXE || req->req_type == IOREQ_READ_NO_RESULT) ? TXN_READ : TXN_WRITE;
...
static int segment_user_request_multi(struct user_request* req)
{
	int lba_seg_count = req->isp_desc->lba_count;

	struct flash_transaction *txn, *tmp;
	int r;
	for (int i = 0; i < lba_seg_count; i++) {
	    unsigned count = 0;
	    struct lba *lba_item = &(req->isp_desc->lba_array[i]);
	    lha_t slba = lba_item->start_lba;
	    //xil_printf("exe_multi: start_lba: %d, sector_count: %d\n", slba, lba_item->sector_count);


	    while (count < lba_item->sector_count) {
	        unsigned page_offset = slba % SECTORS_PER_FLASH_PG;
	        unsigned int txn_size = SECTORS_PER_FLASH_PG - page_offset;
	        lpa_t lpa = slba / SECTORS_PER_FLASH_PG;
	        page_bitmap_t bitmap;

	        if (count + txn_size > lba_item->sector_count)
	            txn_size = lba_item->sector_count - count;
	        ...
	        txn->req = req;
	        txn->type = (req->req_type == IOREQ_READ || req->req_type == IOREQ_EXE || req->req_type == IOREQ_EXE_MULTI || req->req_type == IOREQ_EXE_MULTI_FPGA) ? TXN_READ : TXN_WRITE;
	        txn->source = TS_USER_IO;
	        txn->nsid = req->nsid;
	        txn->lpa = lpa;
	        txn->ppa = NO_PPA;
...
static int process_io_request(struct user_request* req)
{
    ...
    if (req->req_type == IOREQ_EXE_MULTI || req->req_type == IOREQ_EXE_MULTI_FPGA) {
    	// read and parse isp descriptor
    	...
    	for (int n = 0; n < descriptor.lba_count; n++) {
    		xil_printf("lba start: %d, lba count: %d\n", descriptor.lba_array[n].start_lba, descriptor.lba_array[n].sector_count);
    	}*/


    	free_mem(isp_iov[0].iov_base, 4096);
    	req->isp_desc = &descriptor;
    }

    if (req->req_type == IOREQ_EXE_MULTI || req->req_type == IOREQ_EXE_MULTI_FPGA) {
    	r = segment_user_request_multi(req);
    } else {
...
{
	//xil_printf("ftl process request\n");
    int r;
