...
// Allocating memory to heap h
    ...
    int page_num = capacity * sizeof(struct result_item) / 0x1000 + 1;
    h->arr = (struct result_item*) alloc_pages(page_num, ZONE_PS_DDR_LOW);
...
void free_knn_max_heap(struct knn_max_heap *h) {
	...
	int page_num = h->capacity * sizeof(struct result_item) / 0x1000 + 1;
	free_mem(h->arr, page_num * 0x1000);
...
struct buffered_knn_max_heap* create_buffered_knn_max_heap(int capacity, int buffer_factor) {
    ...
    int page_num = sizeof(struct buffered_knn_max_heap) / 0x1000 + 1;
    struct buffered_knn_max_heap* bh = (struct buffered_knn_max_heap*) alloc_pages(page_num, ZONE_PS_DDR_LOW);
    ...
    page_num = bh->buffer_capacity * sizeof(struct result_item) / 0x1000 + 1;
    bh->result_buffer = (struct result_item *) alloc_pages(page_num, ZONE_PS_DDR_LOW);
...
void free_buffered_knn_max_heap(struct buffered_knn_max_heap *bh) {
    ...
    int page_num = bh->buffer_capacity * sizeof(struct result_item) / 0x1000 + 1;
    free_mem(bh->result_buffer, page_num * 0x1000);

    page_num = sizeof(struct buffered_knn_max_heap) / 0x1000 + 1;
    free_mem(bh, page_num * 0x1000);
...
void buffered_knn_max_heap_insert(struct buffered_knn_max_heap *bh, struct result_item *item) {
    ...
    if (unlikely(h->size < h->capacity)) {
        ...
        if (item->distance > split_dist) {
            ...
            // put to buffer
            if (likely(bh->buffer_size < bh->buffer_capacity)) {
...
void init_knn_result_buffer(int k, struct knn_result_buffer *buffer) {
    ...
    int page_num = (k+1) * sizeof(struct result_item) / 0x1000 + 1;

    buffer->result_buffer_k = (struct result_item *) alloc_pages(page_num, ZONE_PS_DDR_LOW);
    buffer->result_buffer_low_half = (struct result_item *) alloc_pages(page_num, ZONE_PS_DDR_LOW);
    buffer->result_buffer_high_half = (struct result_item *) alloc_pages(page_num, ZONE_PS_DDR_LOW);
...
void free_knn_result_buffer(struct knn_result_buffer *buffer) {
	int page_num = (buffer->buffer_capacity+1) * sizeof(struct result_item) / 0x1000 + 1;

    //free(buffer->result_buffer_k);
    free_mem(buffer->result_buffer_k, page_num * 0x1000);
    buffer->result_buffer_k = NULL;
    //free(buffer->result_buffer_low_half);
    free_mem(buffer->result_buffer_low_half, page_num * 0x1000);
    buffer->result_buffer_low_half = NULL;
    //free(buffer->result_buffer_high_half);
    free_mem(buffer->result_buffer_high_half, page_num * 0x1000);
...
void combine_and_sort(struct knn_result_buffer *buffer) {
    ...
    struct result_item *tmp_buffer = (struct result_item *) malloc(tmp_buffer_size * sizeof(struct result_item));;
...
//    h->arr = (struct knnjoin_result_item*)malloc(capacity * sizeof(struct knnjoin_result_item));
    int page_num = capacity * sizeof(struct knnjoin_result_item) / 0x1000 + 1;
    h->arr = (struct knnjoin_result_item*) alloc_pages(page_num, ZONE_PS_DDR_LOW);
...
void free_knnjoin_max_heap(struct knnjoin_max_heap *h) {
    //free(h->arr);
	int page_num = h->capacity * sizeof(struct knnjoin_result_item) / 0x1000 + 1;
	free_mem(h->arr, page_num * 0x1000);
...
bool is_mbr_overlap(struct seg_meta *meta1, struct seg_meta *meta2) {
	bool result = false;
	if (!(meta1->lat_min <= meta2->lat_max && meta1->lat_max >= meta2->lat_min)) {
		return result;
	}
	if (!(meta1->lon_min <= meta2->lon_max && meta1->lon_max >= meta2->lon_min)) {
		return result;
	}
	if (!(meta1->time_min <= meta2->time_max && meta1->time_max >= meta2->time_min)) {
		return result;
	}
	return true;
}
...
void print_heap_runtime_statistics(struct heap_runtime_statistics *statistics) {
	xil_printf("add_to_heap_item_count: %d, add_to_buffer_item_count: %d, discard_count: %d, non_discard_count: %d, heap_sawp_operation_count: %d\n",
			statistics->add_to_heap_item_count, statistics->add_to_buffer_item_count, statistics->discard_count, statistics->non_discard_count, statistics->heap_sawp_operation_count);
}
